...
class Game: 
    ...
    def display(self):
        ...
        for img in self.bgImgs[:]:
            ...
            cnt += 1 
            image(img,space,self.y%self.h,w,self.h) #, space,0,space+w,self.h)
            if self.cnt == 0:
                continue
                #image(img,space,0,w,self.y%self.h, space,self.h-self.y%self.h,space+w,self.h)
            else:
                image(img,space,0,w,self.y%self.h, 0,self.h-self.y%self.h,w,self.h)
            
            #image(img,space,0,w,self.h+vy)
            vy+=5
        self.player.display()
...
class MoveTrain(Train):
    def __init__(self,x,y,w,h,imgName,F):
        Train.__init__(self,x,y,w,h,imgName,F)
...
class Object(Creature): 
    def __init__(self,x,y,r,g,imgName,F):
        Train.__init__(self,x,y,r,g,imgName,F)
        self.vy = 5 #velocity same as the background 
