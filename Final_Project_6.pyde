...
class Player(Creature):
    ...
    def saveandend(self):
        ...
        scoreset=[]
        scoreset.append(game.name+","+str(game.time)+"\n")
        for line in f:
            scoreset.append(line)
        f.close()
        scoreset.sort(key=self.getScore, reverse=True)
        
        f=open("highscores.csv","w")
        for line in scoreset:
            f.write(line)
        print("this is game.time",game.time)
...
def draw():
    ...
    if game.state=='menu':
        ...
        # f.close()
        #name=str(game.name)
        #text(name,game.w//2,game.h//2,300,300)
